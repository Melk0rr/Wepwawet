# INFO: Module to check tls header
...
def check_header(target: "URL") -> Dict:
    ...
    if target.get_ip():
        print(
            f"{target.get_domain()}: checking header with best SSL connection available", end="..."
        )
        ...
        try:
            ...
            ColorPrint.red(f"{target.get_domain()}: error while retrieving Header informations {e}")
